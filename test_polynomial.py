...
from polynomial import Add, Div, Int, Mul, Sub, X
...
def test_mixed_precedence():
    ...
    assert (
        result.i == expected.i
    ), f"Difference of squares failed: {result} != {expected}"
...
def test_representation_accuracy():
    ...
    assert (
        representation == expected_pattern
    ), f"Representation failed: '{representation}' != '{expected_pattern}'"
    ...
    assert (
        representation == expected_pattern
    ), f"Complex representation failed: '{representation}' != '{expected_pattern}'"
...
def run_all_tests():
    ...
    print("=" * 60)
    print(
        "📝 NOTE: This is a student exercise. Some tests will fail until you implement the missing methods."
    )
    print("=" * 60)
